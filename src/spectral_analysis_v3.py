...
nx.draw_networkx_labels(G, pos, ax=ax, font_size=6, font_color="white",
                        font_weight="bold")
ax.set_title("(A)", loc="left", fontsize=10, fontweight="normal", pad=6)
ax.axis("off")
# Legend
...
ax.set_title("(B)", loc="left", fontsize=10, fontweight="normal", pad=6)
...
nx.draw_networkx_labels(G, pos, ax=ax, font_size=6, font_color="white",
                        font_weight="bold")
ax.set_title("(C)", loc="left", fontsize=10, fontweight="normal", pad=6)
ax.axis("off")
cluster_patches = [mpatches.Patch(color=plt.cm.tab10(i), label=f"Cluster {i+1}") for i in range(5)]
ax.legend(handles=cluster_patches, loc="upper left", fontsize=7, framealpha=0.85, title="Cluster")

plt.tight_layout()
fig3.savefig(OUTPUT_DIR + "fig5_spectral_clustering.png", bbox_inches="tight")
plt.close(fig3)
print("Saved: fig5_spectral_clustering.png")


# =============================================================================
# 5b. Figure 7 — Standalone network coloured by cluster membership (NEW)
# =============================================================================
...
intra_w,     inter_w     = [], []
for u, v in G.edges():
    lu = km_labels[node_list.index(u)]
    lv = km_labels[node_list.index(v)]
    w  = G[u][v]["weight"] * 3
    if lu == lv:
        intra_edges.append((u, v)); intra_w.append(w)
    else:
        inter_edges.append((u, v)); inter_w.append(w)

nx.draw_networkx_edges(G, pos, ax=ax7, edgelist=intra_edges,
                       width=intra_w, edge_color="#444444", alpha=0.65)
nx.draw_networkx_edges(G, pos, ax=ax7, edgelist=inter_edges,
                       width=inter_w, edge_color="#bbbbbb", alpha=0.45)
nx.draw_networkx_nodes(G, pos, ax=ax7, nodelist=node_list,
                       node_color=cluster_colors, node_size=400)
nx.draw_networkx_labels(G, pos, ax=ax7, font_size=7,
                        font_color="white", font_weight="bold")

fig7_cluster_patches = [
    mpatches.Patch(color=plt.cm.tab10(i), label=f"Cluster {i+1}")
    for i in range(5)
]
fig7_role_patches = [
    mpatches.Patch(color=c, label=r) for r, c in ROLE_COLORS.items()
]
leg1 = ax7.legend(handles=fig7_cluster_patches, loc="upper left",
                  fontsize=9, framealpha=0.9, title="Cluster")
ax7.add_artist(leg1)
ax7.legend(handles=fig7_role_patches, loc="lower left",
           fontsize=9, framealpha=0.9, title="Community Role")
ax7.axis("off")
plt.tight_layout()
fig7.savefig(OUTPUT_DIR + "fig7_cluster_communities.png", bbox_inches="tight")
plt.close(fig7)
print("Saved: fig7_cluster_communities.png")
# =============================================================================
...
ax8.annotate(f"Seed: {top_node}", xy=(0, diff_history[0]),
             xytext=(0.5, diff_history[0] + 1.5), fontsize=9,
             arrowprops=dict(arrowstyle="->", color="grey"), color="grey")
ax8.annotate(f"72% reach ({diff_history[-1]}/{G.number_of_nodes()} nodes)",
             xy=(8, diff_history[-1]),
             xytext=(5.5, diff_history[-1] - 3), fontsize=9,
             arrowprops=dict(arrowstyle="->", color="grey"), color="grey")
plt.tight_layout()
...
ax9.axvline(x=0.04, color="#c0392b", linewidth=1.0, linestyle=":",
            label="Critical threshold (Δλ₂ = 0.04)")
...
ax5.axis("off")
plt.tight_layout()
fig5.savefig(OUTPUT_DIR + "fig4_network_overview.png", bbox_inches="tight")
plt.close(fig5)
print("Saved: fig4_network_overview.png")
